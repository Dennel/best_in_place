...
BestInPlaceEditor.prototype = {
  ...
  initOptions : function() {
    ...
    self.element.parents().each(function(){
      ...
      self.fire_on_blur     = self.fire_on_blur     || $parent.attr("data-fire-on-blur");
    ...
    self.cancelButtonClass= self.element.attr("data-cancel-button-class") || self.cancelButton;
    ...
    self.fire_on_blur     = self.element.attr("data-fire-on-blur")        || self.fire_on_blur;
  ...
  setHtmlAttributes : function() {
    if(this.formType == ("date" || "time")){
      var formField = this.element.find('input');
    }else{
...
BestInPlaceEditor.forms = {
  "input" : {
    activateForm : function() {
      ...
      this.fire_on_blur = Number(this.fire_on_blur);
    ...
    inputBlurHandler : function(event) {
      if (event.data.editor.okButton && event.data.editor.fire_on_blur) {
        ...
        if (event.data.editor.cancelButton && event.data.editor.fire_on_blur) {
          event.data.editor.blurTimer = setTimeout(function () {
            if (!event.data.editor.userClicked) {
          ...
          if(event.data.editor.fire_on_blur) {
            event.data.editor.update();
          }
        }
      }
    ...
    keyupHandler : function(event) {
      if (event.keyCode == 27) {
        event.data.editor.element.find("input").datepicker('hide');
        event.data.editor.abort();
      }
    }
  },
  ...
  "date" : {
    activateForm : function() {
      ...
      this.element.find('input[name="data"]')
        .datepicker({
            format: 'dd/mm/yyyy'
          })
        .datepicker('show')
        .on('changeDate', function(ev) {
            if (this.okButton == ""){
              that.element.find('input[name="data"]').datepicker('hide');
              that.update(); 
            }
          });
    },
